...
import com.tecgvg.hi.domain.enumeration.HealthCondition;
import com.tecgvg.hi.repository.QuestionsOptionsRepository;
import com.tecgvg.hi.service.criteria.QuestionsOptionsCriteria;
import com.tecgvg.hi.service.dto.QuestionsOptionsDTO;
import com.tecgvg.hi.service.mapper.QuestionsOptionsMapper;
...
class QuestionsOptionsResourceIT {
    ...
    @Test
    @Transactional
    void getAllQuestionsOptionsByQuestionsIsEqualToSomething() throws Exception {
        // Initialize the database
        questionsOptionsRepository.saveAndFlush(questionsOptions);
        Questions questions;
        ...
        em.persist(questions);
        em.flush();
        questionsOptions.setQuestions(questions);
        questionsOptionsRepository.saveAndFlush(questionsOptions);
        Long questionsId = questions.getId();

        // Get all the questionsOptionsList where questions equals to questionsId
        defaultQuestionsOptionsShouldBeFound("questionsId.equals=" + questionsId);

        // Get all the questionsOptionsList where questions equals to (questionsId + 1)
        defaultQuestionsOptionsShouldNotBeFound("questionsId.equals=" + (questionsId + 1));
    }
