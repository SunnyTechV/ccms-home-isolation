package com.tecgvg.hi.web.rest;
...
import com.tecgvg.hi.IntegrationTest;
import com.tecgvg.hi.domain.Assessment;
import com.tecgvg.hi.domain.AssessmentAnswers;
import com.tecgvg.hi.domain.HomeIsolations;
import com.tecgvg.hi.repository.AssessmentRepository;
import com.tecgvg.hi.service.criteria.AssessmentCriteria;
import com.tecgvg.hi.service.dto.AssessmentDTO;
import com.tecgvg.hi.service.mapper.AssessmentMapper;
...
class AssessmentResourceIT {
    ...
    @Test
    @Transactional
    void getAllAssessmentsByAssessmentAnswersIsEqualToSomething() throws Exception {
        // Initialize the database
        assessmentRepository.saveAndFlush(assessment);
        ...
        em.persist(assessmentAnswers);
        em.flush();
        assessment.addAssessmentAnswers(assessmentAnswers);
        assessmentRepository.saveAndFlush(assessment);
        Long assessmentAnswersId = assessmentAnswers.getId();

        // Get all the assessmentList where assessmentAnswers equals to assessmentAnswersId
        defaultAssessmentShouldBeFound("assessmentAnswersId.equals=" + assessmentAnswersId);

        // Get all the assessmentList where assessmentAnswers equals to (assessmentAnswersId + 1)
        defaultAssessmentShouldNotBeFound("assessmentAnswersId.equals=" + (assessmentAnswersId + 1));
    }

    @Test
    @Transactional
    void getAllAssessmentsByHomeIsolationsIsEqualToSomething() throws Exception {
        // Initialize the database
        assessmentRepository.saveAndFlush(assessment);
        ...
        em.persist(homeIsolations);
        em.flush();
        assessment.setHomeIsolations(homeIsolations);
        assessmentRepository.saveAndFlush(assessment);
        Long homeIsolationsId = homeIsolations.getId();

        // Get all the assessmentList where homeIsolations equals to homeIsolationsId
        defaultAssessmentShouldBeFound("homeIsolationsId.equals=" + homeIsolationsId);

        // Get all the assessmentList where homeIsolations equals to (homeIsolationsId + 1)
        defaultAssessmentShouldNotBeFound("homeIsolationsId.equals=" + (homeIsolationsId + 1));
    }
