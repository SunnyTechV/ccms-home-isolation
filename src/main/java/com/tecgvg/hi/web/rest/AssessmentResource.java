...
import com.tecgvg.hi.service.AssessmentQueryService;
import com.tecgvg.hi.service.AssessmentService;
import com.tecgvg.hi.service.criteria.AssessmentCriteria;
import com.tecgvg.hi.service.dto.AssessmentDTO;
import com.tecgvg.hi.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.tecgvg.hi.domain.Assessment}.
 */
@RestController
@RequestMapping("/api")
public class AssessmentResource {
    ...
    public ResponseEntity<List<AssessmentDTO>> getAllAssessments(
        ...
        log.debug("REST request to get Assessments by criteria: {}", criteria);
        Page<AssessmentDTO> page = assessmentQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /assessments/count} : count all the assessments.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/assessments/count")
    public ResponseEntity<Long> countAssessments(AssessmentCriteria criteria) {
        log.debug("REST request to count Assessments by criteria: {}", criteria);
        return ResponseEntity.ok().body(assessmentQueryService.countByCriteria(criteria));
    }
